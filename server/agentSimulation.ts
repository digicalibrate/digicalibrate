...
const synthesizedThreads = new Set<number>();

export function initAgentSimulation(wsServer: WebSocketServer) {
  wss = wsServer;
...
async function runSimulationStep() {
  try {
    ...
    console.error("[AgentSimulation] Error:", err);
  }
  if (simulationRunning) {
    scheduleNext();
  }
...
async function postProposal() {
  const agent = pickRandom(SIM_AGENTS);
...
async function postCritique(parentMsg: any) {
  const availableAgents = SIM_AGENTS.filter((a) => a.name !== parentMsg.agentName);
...
async function postExpansion(parentMsg: any) {
  const usedNames = [
    parentMsg.agentName,
    ...(parentMsg.echoes || []).map((e: any) => e.agentName),
  ];
  const availableAgents = SIM_AGENTS.filter((a) => !usedNames.includes(a.name));
  const agent = pickRandom(availableAgents.length > 0 ? availableAgents : SIM_AGENTS);
...
async function postSynthesis(parentMsg: any) {
  synthesizedThreads.add(parentMsg.id);
